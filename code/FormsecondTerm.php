...
?>
<?php
$formtitle = "Second Term";
$formname  = "FormsecondTerm.php";
?>
<!doctype html>
<html lang="en">
  ...
  ?>
    ...
    ?>
      ...
      if (isset($_POST['removemark'])) {
        ...
        echo  $changedSQL = "DELETE FROM `marks_second_term` where  `subjectid` = '$var2' and `class` = '$var1' and `subclassname` = '$var3' and `acadmicyear` = '$Acadyear'";
        ...
        if ($_POST['Addmark']) {
          ...
          for ($z = 0; $z < $max; $z++) {
            ...
            //$Acadyear = "2017-2018";
               ...
               {
               $changedSQL = "delete from `marks_second_term` where `studentindex` = '$studentidfromarray' and
...
{
            $mark =            getmark2nd($studentidfromarray, $subjectid, $classid, $subclassname, $Acadyear);
            if ($mark == "**") {
              $changedSQL = "INSERT INTO `marks_second_term`( `studentindex`,`subclassname`, `subjectid`, `class`, `mark`, `acadmicyear`, `date`) VALUES ('$studentidfromarray','$subclassname','$subjectid','$classid','$studentmarkfromarray','$Acadyear',now())";
              mysqli_query($link, $changedSQL);
            } else {
              $changedSQL = "UPDATE `marks_second_term` set `mark` = '$studentmarkfromarray' where `studentindex` = '$studentidfromarray' and `subclassname` = '$subclassname' and `subjectid` = '$subjectid' and `class` = '$classid' and `acadmicyear` = '$Acadyear'";
              mysqli_query($link, $changedSQL);
            }
          }
          }
              ...
              <td>
                <div class="col-md-6">
                  <input name="termmark<?php echo $studentindex; ?>" value="<?php
                                                                              ...
                                                                              $first = getmark2nd($studentindex, $var2, $var1, $var3, "$Acadyear");
                                                                              if ($first != '**')
                                                                                echo intval($first);
