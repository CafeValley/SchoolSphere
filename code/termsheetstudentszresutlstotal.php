...
$formname  = "termsheetstudentszresutlstotal.php";
    ...
    <div class="box-body" style="font-family:'Droid Arabic Naskh', serif;">
      ...
      <div id="printableArea" class="table-responsive">
        ...
        if (isset($_POST['Getit'])) {
          ...
          echo "<br>";
          if ($souceofdata == 'register') {
            $stndlist = getstndFresults($grade, $subclassname, 'register');
          } else
            $stndlist = getstndFresults($grade, $subclassname);
          ...
          <table id="myTable" class="table table-striped table-sm">
            ...
            <tbody>
              ...
              while ($CheckIfReturnInF   = mysqli_fetch_array($CheckIfHereInF)) {
                ...
                if ($souceofdata == 'register') {
                  $tfirst = getfirstterm($grade, $subclassname, $studentindex, 'register');
                  $t2nd = get2ndterm($grade, $subclassname, $studentindex, 'register');
                  $t3rd = get3rdterm($grade, $subclassname, $studentindex, 'register');
                  $avghere = getaverage($tfirst, $t2nd, $t3rd);
                  $final = getfinalterm($grade, $subclassname, $studentindex, 'register');
                } else {
                  $tfirst = getfirstterm($grade, $subclassname, $studentindex);
                  $t2nd = get2ndterm($grade, $subclassname, $studentindex);
                  $t3rd = get3rdterm($grade, $subclassname, $studentindex);
                  $avghere = getaverage($tfirst, $t2nd, $t3rd);
                  $final = getfinalterm($grade, $subclassname, $studentindex);
                }

                $Fregus = getaverage($avghere, $final, '**');;
                echo "<td>" . mynumberformat($tfirst, 0) . "</td>";
                echo "<td>" . mynumberformat($t2nd, 0) . "</td>";
                echo "<td>" . mynumberformat($t3rd, 0) . "</td>";
                echo "<td>" . mynumberformat($avghere, 0) . "</td>";
                echo "<td>" . mynumberformat($final, 0) . "</td>";
                echo "<td>" . mynumberformat($Fregus, 0) . "</td>";
