...
?>
<?php
$formtitle = "First Term";
$formname  = "FormFirstTerm.php";
?>
<!doctype html>
<html lang="en">
  ...
  <main role="main" class="col-md-9 ml-sm-auto col-lg-10 pt-3 px-4">
    ...
    <div class="table-responsive" id="printableArea">
      ...
      if (isset($_POST['removemark'])) {
        ...
        echo  $changedSQL = "DELETE FROM `marks_first_term` where  `subjectid` = '$var2' and `class` = '$var1' and `subclassname` = '$var3' and `acadmicyear` = '$Acadyear'";
        ...
        if ($_POST['Addmark']) {
          ...
          for ($z = 0; $z < $max; $z++) {
            ...
            if ($studentmarkfromarray == '-1') {
              $changedSQL = "delete from `marks_first_term`where `studentindex` = '$studentidfromarray' and
            `subclassname` = '$subclassname' and `subjectid` = '$subjectid' and `class` = '$classid' and `acadmicyear` =
            '$Acadyear'";
              mysqli_query($link, $changedSQL);
            } else {
              $mark =            getmarkfirst($studentidfromarray, $subjectid, $classid, $subclassname, $Acadyear);
              if ($mark == "**") {
                $changedSQL = "INSERT INTO `marks_first_term`( `studentindex`,`subclassname`, `subjectid`, `class`, `mark`, `acadmicyear`, `date`) VALUES ('$studentidfromarray','$subclassname','$subjectid','$classid','$studentmarkfromarray','$Acadyear',now())";
                mysqli_query($link, $changedSQL);
              } else {
                $changedSQL = "UPDATE `marks_first_term` set `mark` = '$studentmarkfromarray' where `studentindex` = '$studentidfromarray' and `subclassname` = '$subclassname' and `subjectid` = '$subjectid' and `class` = '$classid' and `acadmicyear` = '$Acadyear'";
                mysqli_query($link, $changedSQL);
              }
            }
          }
                ...
                <td>
                  <div class="col-md-6">
                    <input name="termmark<?php echo $studentindex; ?>" value="<?php
                                                                              ...
                                                                              $first = getmarkfirst($studentindex, $var2, $var1, $var3, "$Acadyear");
                                                                              if ($first != '**')
                                                                                echo intval($first);
