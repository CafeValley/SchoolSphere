...
$SqlCommandForCFe2 = "SELECT `StudentName`, CONCAT('| ',studentindex ,' |')  as 'Index Number' ,sum(totaloffees) as Due , studentindex as studentindexbasic
  ...
  ($studentlist)  and studentsregisteredfees.acadmicyear = '$Acadyear' group by studentindex , StudentName order by
  `StudentName`";
...
while ($CheckIfReturnInF2 = mysqli_fetch_array($CheckIfHereInF2, MYSQLI_ASSOC)) {
  ...
  $rowData[] = $newarry;
  $totaldue += $CheckIfReturnInF2['Due'];;
  $totalpaid += getamountpaied($studnetindex);
  $totalbalance += intval($CheckIfReturnInF2['Due'] - getamountpaied($studnetindex));
}
$newarry['No'] = " ";
$newarry['Student Name'] = " ";
$newarry['Student Index'] = " ";
$newarry['Due'] = number_format($totaldue);
$newarry['Paid'] = number_format($totalpaid);
$newarry['Balance'] = number_format($totalbalance);

$rowData[] = $newarry;

//print_r( $rowData);

$fileName = "Students Payments" . date('Ymd') . ".xls";
